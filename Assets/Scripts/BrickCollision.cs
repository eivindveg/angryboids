	...
	public class BrickCollision : MonoBehaviour
	{
		...
		private void OnCollisionEnter2D(Collision2D collision)
		{
			...
			if (magnitudeMass >= 1.5)
			{
				...
				if (this.Type == BrickType.Block && Time.timeSinceLevelLoad > 2f)
				{
					int soundIndex = Random.Range(0, this.CollisionSound.Length);
					this.audio.PlayOneShot(this.CollisionSound[soundIndex]);
				}
			}
